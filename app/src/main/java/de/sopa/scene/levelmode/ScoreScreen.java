...
public class ScoreScreen extends BaseScene {
    ...
    ScoreScreen(final LevelResult levelResult) {
        ...
        Text levelCompleteTextShape = new Text((float) (camera.getWidth() * 0.05), (float) (camera.getHeight() * 0.05),
                resourcesManager.movesScoreFont, levelCompleteString, vbom);
        levelCompleteTextShape.setScaleCenter(0, 0);
        levelCompleteTextShape.setScale(2);
        attachChild(levelCompleteTextShape);
        attachChild(new Text((float) (camera.getWidth() * 0.05), (float) (camera.getHeight() * 0.4),
                resourcesManager.movesScoreFont,
                "Your moves:          \t" + levelResult.getMoveCount() + "\nMoves for 3 Stars:\t"
                + level.getMinimumMovesToSolve(), vbom));
        attachChild(new Sprite(0, (float) (camera.getHeight() * 0.55), 400, 400, resourcesManager.starRegion, vbom));
        attachChild(new Sprite((float) (camera.getWidth() * 0.64), (float) (camera.getHeight() * 0.55), 400, 400,
                (levelResult.getStars() == 3) ? resourcesManager.starRegion : resourcesManager.starSWRegion, vbom));
        attachChild(new Sprite((camera.getWidth() / 2 - 200), (float) (camera.getHeight() * 0.6), 400, 400,
                (levelResult.getStars() >= 2) ? resourcesManager.starRegion : resourcesManager.starSWRegion, vbom));

        ButtonSprite choiceLevelButton = new ButtonSprite(0, (camera.getHeight() - 400),
                resourcesManager.backToChoiceRegion, vbom, new ButtonSprite.OnClickListener() {
        ...
        ButtonSprite nextLevelButton = new ButtonSprite((float) (camera.getWidth() * 0.64), (camera.getHeight() - 400),
                resourcesManager.nextLevelRegion, vbom, new ButtonSprite.OnClickListener() {
        ...
        ButtonSprite levelAgainButton = new ButtonSprite((camera.getWidth() / 2 - 200), (camera.getHeight() - 400),
                resourcesManager.backToMenuRegionA, vbom, new ButtonSprite.OnClickListener() {
